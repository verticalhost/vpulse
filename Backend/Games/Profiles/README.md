...
`delta-force.json` covers the same top-right feed. Its region was generated by the local
vision model in `tools/calibrate-game` and beat the hand-drawn one on a live session with known
kills: 3 of 3 found against 2 of 3, because its taller span catches rows deep in a stacked feed.
A caution that came out of the same test: region edits are not monotonic — a merged region
strictly larger than the hand-drawn one found FEWER kills, because extra area shifts the OCR's
row grouping. Change a region only against a recording with known kills, never by eye.
